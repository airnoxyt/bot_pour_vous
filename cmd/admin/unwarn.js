...
module.exports.run = (client , message, args) => {
    ...
    if(!user){
        ...
        if(!db[server][user.id] || db[user.id].nombre === 0){
            message.channel.send(`${message.author} , ${user} n'a jamais été warn donc on ne peut pas lui enlever un warn`)
            return;
      }if(db[server][user.id].nombre = -1){
        db[server][user.id].nombre = 0
      }
      let dbn = db[server][user.id].nombre;
       db[server][user.id].nombre = dbn - 1
    ...
    fs.writeFile("./warn.json", JSON.stringify(db, null , 4), (err) => {
        if(err) console.log(err)
    })
